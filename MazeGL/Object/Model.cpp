...
namespace objloader
{
    ...
    void Model::processMaterialLine(const std::string& line)
    {
        ...
        else if (line.find("map_Ka") == 0) // Texture mapping
        {
            std::vector<std::string> tokens;
            split(line, ' ', tokens);
		...
		else if (line.find("map_Ks ") == 0) // Texture mapping
		{
			std::vector<std::string> tokens;
			split(line, ' ', tokens);
...
#else
                glNormal3fv(object_.normals.at(item.normalIndex).coords);
#endif
//MDN
                if (item.textureIndex > 0) { // Texture coordinates are provided
                    ...
                    /*if(material.texScaleU!=0 && material.texScaleV!=0)
